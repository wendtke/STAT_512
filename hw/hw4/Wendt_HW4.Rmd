...
As indicated above, the slope for precipitation in secondary forest (`age` 1) differs from 0.
...
bio_slope_em <- emmeans::emtrends(model = bio_ancova_alt,
                                  specs = "age",
                                  var = "ppt")
...
> For Question 7,  in doing the backwards elimination,  when I used the code that was in the Highway example, the summary leaves out the first level of the term of Age in both the main effects and the interaction with Ppt.  Does this matter, because the term Age has to be included or excluded at all levels anyway?

> You are right -- age has to be included or excluded at all levels.   This is why for categorical variables with more than 2 levels, you'll want to refer to the Anova Type 3 results, rather than the coefficient table from summary().   The Anova table tests all levels simultaneously. 
