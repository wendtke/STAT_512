...
kableExtra::kable(bio_water_pairs)
```
...
No interaction indicated at $\alpha$ = 0.05.
...
Residuals vs Fitted: Residuals evenly distributed across level but still some cluster. Assumptions of linearity and constant variance seem reasonable.
...
kableExtra::kable(forb_water_pairs)
```

## Question 8: LSD - REVISIT
...
kableExtra::kable(forb_water_pairs_avg)
```

## Question 10: LSD - REVISIT
...
As sample size increases and margin of error decreases, power increases. 
...
```{r 5_grass_anova}
# 5. subset data to grass type only
grass_data <- bio_data %>% dplyr::filter(type == "Grass")
# 5. adjust contrast default
options(contrasts = c("contr.sum", "contr.poly"))
# 5. fit two-way anova model for grass data
...
# 6. diagnostic plots for two-way anova model of type grass
plot(grass_anova)
```
...
# 7. kable emmeans and contrasts
kableExtra::kable(grass_water_pairs)
```
...
No, it would not be appropriate to average over herbicide because there is evidence of an interaction between water patter and herbicide among plant type `grass`.
...
The combined model (#4) had 24 degrees of freedom, whereas the split-type models (#7 and #14) each had 12 degrees of freedom. Split model 1 for `forb` (#7) had the smallest standard errors, but split model 2 for `grass` (#14) had the largest standard errors. The estimated marginal means and estimated differences across the models (i.e., combined vs. split 1; combined vs. split 2) were the same.
...
Splitting the data reduces dfResid, therefore reducing power. In this case, the three-way model had 24 dfResid, but each two-way model only had 12 dfResid.
